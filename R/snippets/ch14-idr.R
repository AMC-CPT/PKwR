# 간접반응(생성 억제): dR/dt = kin (1 - Cp/(IC50 + Cp)) - kout R   (Imax = 1)
# 비선형 미분방정식의 일반 해법인 4차 Runge-Kutta 를 그대로 구현하여 푼다
rk4 <- function(f, y0, t) {
...
didr <- function(D) function(t, R) {                  # 용량별 미분식
  Cp <- D/V*exp(-k*t)
  kin*(1 - Cp/(IC50 + Cp)) - kout*R
}
t  <- seq(0, 96, 0.05)
R1 <- rk4(didr(100), 100, t)                          # 기저치 R0 = kin/kout = 100
