# 가중치: 무엇을 '똑같이 중요한 오차'로 볼 것인가
wsse <- function(p, w) { f <- p[1]*exp(-p[2]*d4$x); sum(w(f)*(d4$DV - f)^2) }
w.ols <- function(f) 1                        # 등가중 (절대오차 가정)
w.wls <- function(f) 1/f^2                    # 1/f^2 가중 (비례오차 가정)
els   <- function(p) { f <- p[1]*exp(-p[2]*d4$x); v <- (p[3]*f)^2
                       sum(log(v) + (d4$DV - f)^2/v) }   # 분산도 추정한다
