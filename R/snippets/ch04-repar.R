# 재모수화: 모형은 그대로 두고 좌표만 바꾼다. SSE 는 같지만 성질이 달라진다.
t0 <- sum(d4$x*exp(-2*pe4[2]*d4$x))/sum(exp(-2*pe4[2]*d4$x))   # 자료의 무게중심
mods <- list(
...
round(rbind(Wald.on.k     = pe4[2] + c(-1, 1)*tc*se4[2],
            ...
            profile       = ci.k), 4)
asym <- function(ci, est) (ci[2] - est)/(est - ci[1])       # 위쪽/아래쪽 폭의 비
