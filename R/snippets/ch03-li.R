...
library(LBI)                                  # 저자의 가능도 기반 추론 패키지
n1 <- length(lh); v1 <- var(lh); cl <- 0.95
ci  <- (n1 - 1)*v1/qchisq(c(0.5 + cl/2, 0.5 - cl/2), n1 - 1)   # 통상의 카이제곱 CI
lb  <- LBCIvar(lh,   conf.level = cl)["var", c("LL", "UL")]    # 가능도기반 CI
li  <- LInormVar(lh, conf.level = cl)["var", c("LL", "UL")]    # 가능도구간
w   <- rbind(CI = ci, LBCI = lb, LI = li)
round(cbind(w, width = w[, 2] - w[, 1]), 4)

# 왜 다른가: 가능도 곡선 위에서 두 끝점의 높이를 비교한다
LL <- function(v) -n1/2*(log(2*pi*v) + (n1 - 1)*v1/(n1*v))     # 프로파일 로그가능도
