# 약물을 바꾸려면 모형 목록과 목표만 바꾼다. 엔진(addStop, pred2c, fitEBE)은 그대로다.
# 아래 값은 자릿수만 문헌에 맞춘 예시다. 실제로는 자기 집단의 NONMEM 출력을 넣는다.
MODELS <- list(
...
ssM <- function(pk, Dose, tau, Tinf) with(pk, {   # 2구획 반복 주입의 항정상태
...
doseFor <- function(m, eta, CLcr) {               # 목표에 맞춘 1회 용량
