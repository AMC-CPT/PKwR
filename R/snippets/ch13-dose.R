# 용량 조정: 목표 평균 항정농도 Cav = 10 mg/L, 투여간격 12 h.
# 유지용량 = Cav x CL x tau. 경구 사전이므로 CL 은 CL/F 이고,
# 나오는 용량은 그대로 '경구 용량'이다.
CL.map <- TH[[2]]*exp(fit5[2])*TH[[3]]*exp(fit5[3])
D.pop <- 10*TH[[2]]*TH[[3]]*12                      # 사전(집단)만 알 때의 용량
D.new <- 10*CL.map*12                               # MAP 개인 CL 로 조정한 용량
round(c(CL.pop = TH[[2]]*TH[[3]], CL.map = CL.map,
        D.pop = D.pop, D.new = D.new), 1)

# 항정상태 예측 (1구획 경구 반복투여의 해석해)
...
# 조정 후 실제(참값) 평균농도: 목표 10 에 얼마나 가까운가
