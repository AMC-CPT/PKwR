# nmw 로 검산: NONMEM 의 $EST MAXEVAL=0 POSTHOC 에 해당한다. 집단 추정
# (EstStep) 없이 사전값을 FinalPara 에 넣고 PostHocEta 만 부른다.
library(nmw)
e <- get("e", envir = asNamespace("nmw"))
FGD <- deriv(~ DOSE/(TH2*exp(ETA2)) *               # 10장의 PRED 그대로
...
e$FinalPara <- c(TH, mat2ltv(OM), SG)               # 추정 없이 사전을 주입
