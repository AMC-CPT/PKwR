# 사전(prior): 10장에서 Theoph 자료로 얻은 FOCE-I 최종 추정치를 그대로 쓴다.
# theophylline 경구 1구획: KA, V, K (겉보기 경구 파라미터, F 는 V 와 CL 에 흡수)
TH <- c(KA = 1.4872, V = 31.9982, K = 0.0872)
OM <- matrix(c( 0.4248, 0.0321, -0.0031,            # eta 공분산 (full block)
                0.0321, 0.0158,  0.0126,
               -0.0031, 0.0126,  0.0201), 3, 3)
SG <- c(prop = 0.0180, add = 0.0740)                # 잔차 분산 (비례, 가법)
