# Interspecies scaling: clearance follows a power of body weight.  CL = a * W^b
W  <- c(mouse = 0.02, rat = 0.25, rabbit = 2.5, monkey = 5, dog = 12)  # kg
CL <- c(0.099, 0.77, 3.7, 7.3, 12.2)                                   # L/hr

fit <- lm(log(CL) ~ log(W))
a <- exp(coef(fit)[[1]]); b <- coef(fit)[[2]]
CL.human <- a * 70^b                    # predicted clearance of a 70 kg adult

# Daily dose reproducing the exposure effective in animals (AUC 5 mg*hr/L) = CL * AUC
