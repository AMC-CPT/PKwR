# Profile: fix one parameter, re-optimize the rest, and follow the SSE
pe4 <- fit.ols$par; n4 <- nrow(d4); s2 <- fit.ols$value/(n4 - 2)
V4  <- Jac(pe4); se4 <- sqrt(diag(s2*solve(t(V4) %*% V4)))    # Wald standard errors
tau <- function(idx, v) {
