# The FOCE-I objective function by hand. Only the EBEs and final estimates are needed.
...
OFVi <- sapply(seq_len(nrow(EBE)), function(i) {
  ...
  Gi <- M[, c("G1", "G2", "G3")]
  Ri <- Di$DV - M[, "F"]
  Phi <- sum(log(Vi) + Ri^2/Vi)              # individual -2LL (constant omitted)
  Phi + drop(t(et) %*% iOM %*% et) + lgOM +  # prior (penalty) term + log|OMEGA|
...
round(head(OFVi, 4), 3)                      # individual contributions to the OFV
